-- Restore CO2 impact RPC after revoking anon SELECT on co2_public_stats.
-- Uses SECURITY DEFINER so it can read the tiny cache row while the underlying
-- table remains inaccessible to anon/authenticated via GraphQL/PostgREST.
...
-- Refresh cache in case triggers hadn't fired since last data change
